...
class SlewBearing:
    ...
    def __init__(self, outer_diameter, inner_diameter, width, roller_fit, roller_slide, num_rollers):
        ...
        if any(map(lambda x: x < 0.0, (inner_radius, outer_radius, width, roller_fit, roller_slide, num_rollers))):
            raise NegativeValueError('Value can\'t be less than 0.0.')

        if num_rollers % 2 == 1:
            raise OddRollerValueError('The number of rollers must be divisible by 2.')
        ...
        if outer_radius**2.0 + inner_radius**2.0 - (roller_diameter + roller_fit)**2.0 <= 0.0:
            raise PitchRadiusValueError('Pitch radius not real.')
        ...
        if pitch_radius - inner_radius - (roller_diameter + roller_fit) * sqrt(2.0)/2.0 < inner_race_min_thickness:
            raise InnerRaceValueError('Inner race min thickness too low. Consider decreasing inner diameter, increasing outer diameter, decreasing roller fit or increasing the number of rollers.')
        ## outer race as single piece
        if outer_radius - pitch_radius - (roller_diameter + roller_fit) * sqrt(2.0)/2.0 < outer_race_min_thickness:
            raise OuterRaceValueError('Outer race min thickness too low. Consider increasing outer diameter, decreasing inner diameter, decreasing roller fit or increasing the number of rollers.')
        ...
        if roller_chamfer_length < roller_chamfer_min_length:
            raise RollerChamferValueError('Roller chamfer too small. Consider increasing roller fit, decreasing roller slide, decreasing bearing width or decreasing the number of rollers.')
...
while True:
    try:
        ...
        print(str(NR) + " doesn't work " + str(e))
        NR += 2
while True:
    try:
